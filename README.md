...
<b>사전에 준비할 것</b>
    ...
    영화의 일일 박스오피스 순위의 정보의 오픈API URL과 Key를 "영화권입장권통합전산망"에서 제공받는다.
...
이 후 웹으로 응답 받은 JSON 결과물을 처리하기 위해 gson을 라이브러리에 추가하고 JSON을 자바 객체로 바꿀 때 필요한 MovieList 라는 클래스를 만들어준다.
...
이제 이것들을 화면에 리스트 모양으로 보여주기 위한 작업으로 activity_main.xml 파일에 recyclerview를 추가하고 이를 관리할 MovieAdapter를 클래스를 생성한다.

<b>[실제MovieAdapter에 적은 코드]</b>
...
결국 ViewHolder안에는 Movie의 객체들이 담기게 되며 setItem()메서드로 인해 Movie 객체를 전달한다.
...
리싸이클러뷰를 위한 어댑터가 만들어졌으면 MainActivity로 돌아와 어댑터를 리싸이클러뷰 객체에 설정하고 그 아래에 MovieAdapter 객체를 만들어 setAdapter()를 호출하도록 설정하면recyclerview와 adater가 상호작용하면서 리스트모양으로 보여준다.
...
마지막으로 사용자가 버튼을 눌러 응답을 받았을 때 호출되는 OnResponse() 는 processResponse()를 호출한다. 이 processResponse() 안에서는 Gson을 이용해 Json 문자열을 MovieList 객체로 변환하며 그 안에 들어있는 Movie 객체들을 하나씩 꺼내어 어댑터에 추가한다. 어댑터를 모두 추가했다면 마지막 줄에 있는 notifyDataSetChanged()를 호출해야 변경 사항이 반영되고리싸이클러뷰에 영화 정보가 표시된다.
...
이 프로젝트에서 사용한 TextView는 총 6개로 프로젝트 명, 영화순위, 영화제목, 개봉일, 일별 관람객 수, 누적 관람객 수를 보여준다. 프로젝트 명을 제외한 나머지는 ViewHolder안에 담긴 Movie의 객체들을 전달 받아 TextView로 표시를 해주는 방식이고 이를 화면에 구현 하기 위해서는 MovieAdapter.java 페이지에서 작성하며 findViewById(R.id.textview); 코드를 입력해 xml과 연결시켜야 한다. 
...
button1은 kobis mobile 페이지로, button2는 kobis mobile 최신상영작 페이지로 이동하는데 구현하는 방법은 OnClick 함수를 지정하는 것이다. 프로그램에서 button1의 OnClick  함수는 B1 으로, button2의 OnClick 함수는 B2로 지정했다. 그 후 MainActivity로 이동해 button1의 OnClick의 함수인 B1을 호출하고 Intent 함수를 myIntent로 불러오는 위와 같은 코드를 작성한다. button2도 같은 방법으로 작성 가능하다.
...
이 프로젝트 이름인 Daily Box Office TOP10을 맨 위 상단에 textview로 나타내고 기존에 길고 어딘가 답답해보였던 요청하기 버튼을 돋보기 모양으로 된 작은 imageButton으로 깔끔하게 표현했다.
...
기존에는 영화의 제목과 관객수만 보이는 단순한 구조였지만 순위와 개봉일을 추가하고 일별 관람객과 누적 관람객으로 나눠 표시했다. 
그 후 모바일 홈페이지와 최신상영작페이지로 바로갈 수 있는 버튼 두 개를 추가하여 깔끔하면서도 사용자들이 편리하게 사용할 수 있도록 만들었다.
